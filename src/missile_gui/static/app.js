...
function buildCharts(result) {
  ...
  const configs = [
    ...
    ["#chart-g", { xLabel: "时间 (s)", yLabel: "过载 (G)", zeroY: true, markers: result.markers, datasets: [{ label: "指令 G", color: COLORS.missile, points: timeSeries((s) => Math.hypot(...s.commanded_acceleration_g)) }, { label: "实际 G(轨迹法向,含推力分量,与游戏屏显同基)", color: COLORS.actual, points: timeSeries((s) => s.trajectory_lateral_load_g ?? s.actual_overload_g) }] }]
  ];
